...
export const signUp = async (req, res) => {
    try {
        ...
        if (!username || !password || !email || !name) {
            return res.status(400).json({ message: 'Khong the thieu username, password, email hoac name' });
        }
...
export const signIn = async (req, res) => {
    try {
        ...
        const accessToken = jwt.sign({ user_id: user.user_id}, process.env.ACCESS_TOKEN_SECRET, { expiresIn: ACCESS_TOKEN_TTL });
...
export const signOut = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: 'Loi server khi dang xuat', error: error.message });
    }
}