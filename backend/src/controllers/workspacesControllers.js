...
export const getWorkspaces = async (req, res) => {
    try {
        ...
        if (!user_id) return res.status(401).json({ error: "Không xác định được người dùng" });
...
export const createWorkspaces = async (req, res) => {
    try {
        ...
        if (!name || typeof name !== 'string' || name.trim() === '') {
            return res.status(400).json({ error: 'Tên Workspace là bắt buộc và không được để trống' });
        }
        if (!slug || !slugRegex.test(slug)) {
            return res.status(400).json({ error: 'Slug không hợp lệ (chỉ chấp nhận chữ thường, số và dấu "-")' });
        }

        const safeDescription = (description && typeof description === 'string') ? description.trim() : null;

        const workspace = await createWorkspace(name.trim(), safeDescription, slug.trim(), owner_id);
        res.status(201).json(workspace);

    } catch (error) {
        if (error.code === '23505') {
            return res.status(409).json({ error: 'Slug này đã tồn tại, vui lòng chọn đường dẫn khác' });
        }
...
export const updateWorkspaces = async (req, res) => {
    try {
        ...
        if (!name && description === undefined) {
            return res.status(400).json({ error: 'Cần cung cấp ít nhất tên hoặc mô tả để cập nhật' });
        }
        ...
        const updatedWorkspace = await updateWorkspace(workspace_id, name?.trim(), description);
        if (!updatedWorkspace) {
            return res.status(404).json({ error: 'Không tìm thấy Workspace để cập nhật' });
        }   
