...
export const getSpaceOverview = async (space_id) => {
    try {
        const query = `
            SELECT 
                (SELECT COUNT(*) FROM tasks WHERE space_id = $1 AND is_archived = FALSE) as total_tasks,
                (SELECT COUNT(*) FROM tasks t JOIN task_status ts ON t.status_id = ts.status_id 
                 WHERE t.space_id = $1 AND ts.is_done_state = TRUE AND t.is_archived = FALSE) as completed_tasks,
                (SELECT COUNT(*) FROM tasks WHERE space_id = $1 AND due_date < CURRENT_DATE 
                 AND is_archived = FALSE AND completed_at IS NULL) as overdue_tasks,
                (SELECT COUNT(*) FROM tasks WHERE space_id = $1 AND is_archived = FALSE 
                 AND completed_at IS NULL) as in_progress_tasks,
                (SELECT COUNT(DISTINCT ta.user_id) FROM task_assigns ta 
                 JOIN tasks t ON ta.task_id = t.task_id 
                 WHERE t.space_id = $1) as total_members,
                (SELECT COALESCE(SUM(tl.duration_secs), 0) FROM time_logs tl 
                 JOIN tasks t ON tl.task_id = t.task_id 
                 WHERE t.space_id = $1 AND tl.stopped_at IS NOT NULL) as total_time_secs
        `;
        const values = [space_id];
        const result = await con.query(query, values);
...
export const getWorkspaceOverview = async (workspace_id) => {
    try {
        const query = `
            SELECT 
                (SELECT COUNT(*) FROM spaces WHERE workspace_id = $1) as total_spaces,
                (SELECT COUNT(*) FROM tasks t 
                 JOIN spaces s ON t.space_id = s.space_id 
                 WHERE s.workspace_id = $1 AND t.is_archived = FALSE) as total_tasks,
                (SELECT COUNT(*) FROM tasks t 
                 JOIN spaces s ON t.space_id = s.space_id 
                 JOIN task_status ts ON t.status_id = ts.status_id
                 WHERE s.workspace_id = $1 AND ts.is_done_state = TRUE AND t.is_archived = FALSE) as completed_tasks,
                (SELECT COUNT(DISTINCT wm.user_id) FROM workspace_members wm 
                 WHERE wm.workspace_id = $1) as total_members
        `;
