...
const STATUS_OPTIONS = [
    { id: 1, label: 'TO DO', color: '#9ca3af' },
    { id: 2, label: 'IN PROGRESS', color: '#2563eb' },
    { id: 3, label: 'COMPLETE', color: '#22c55e' }
];
...
export default function CreateTaskModal({
    ...
    }, [isOpen, defaultStatusId, defaultListId, lists]);

    if (!isOpen) return null;

    const currentStatus = STATUS_OPTIONS.find(s => s.id === statusId) || STATUS_OPTIONS[0];
    const currentPriority = PRIORITY_OPTIONS.find(p => p.id === priorityId) || PRIORITY_OPTIONS[2];
    ...
    return (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
            <div
                className="flex w-[580px] max-w-[95vw] flex-col rounded-xl bg-white shadow-2xl"
            ...
            >
                ...
                <div className="px-6 py-4">
                    <input
                        ref={titleRef}
                        className="mb-2 w-full border-none text-xl font-bold text-[#141b2b] outline-none placeholder:text-[#c2c9e0]"
                        value={name}
                    ...
                    <div className="flex items-start gap-2">
                        ...
                        <textarea
                            className="w-full resize-none border-none text-[13px] leading-6 text-[#5f6368] outline-none placeholder:text-[#c2c9e0]"
                            value={description}
                ...
                <div className="flex flex-wrap items-center gap-2 border-t border-[#f8f9fc] px-6 py-3">
                    ...
                    <div className="relative" onClick={e => e.stopPropagation()}>
                        <button className="flex items-center gap-1.5 rounded-full border border-[#eef0f5] px-3 py-1.5 text-xs font-semibold hover:bg-[#f8fafb]" style={{ color: currentPriority.color }} onClick={() => setActiveDropdown('priority')}>
                            <Flag size={14} fill={priorityId ? currentPriority.color : 'none'} /> {currentPriority.label}
                        </button>
                        {activeDropdown === 'priority' && (
                            <div className="absolute bottom-full left-0 mb-1 w-40 rounded-lg border border-[#eef0f5] bg-white p-1 shadow-xl">
                                {PRIORITY_OPTIONS.map(p => (
                                    <button key={p.label} className="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-xs font-medium hover:bg-[#f0f4ff]" onClick={() => { setPriorityId(p.id); setActiveDropdown(null); }}>
                                        <Flag size={13} color={p.color} fill={p.id ? p.color : 'none'} /> {p.label}
                                    </button>
                                ))}
                            </div>
                        )}
