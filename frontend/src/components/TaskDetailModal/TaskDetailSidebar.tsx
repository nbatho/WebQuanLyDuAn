...
export default function TaskDetailSidebar({
    ...
    return (
        <div className="flex w-65 shrink-0 flex-col gap-4 overflow-y-auto border-l border-[var(--color-border-light)] p-4">
            <div className="flex flex-col gap-1">
                {/* Assignee */}
                <div className="relative flex flex-col gap-0.75 py-1.5" ref={assigneeRef}>
                    <span className="text-[11px] font-bold uppercase tracking-[0.04em] text-[var(--color-text-tertiary)]">Assignee</span>
                    <div
                    ...
                    >
                        {task.assignees && task.assignees.length > 0 ? (
                            ...
                            <>
                                <User size={14} className="shrink-0 text-[var(--color-text-tertiary)]" /> Assign
                            </>
                        )}
                    </div>
                    {showAssignee && (
                        <div className="absolute right-0 top-[calc(100%+4px)] z-20 min-w-45 rounded-lg border border-[var(--color-border-light)] bg-white p-1 shadow-[0_6px_20px_rgba(0,0,0,0.12)]">
                            {MEMBER_OPTIONS.map((m) => (
                                <button
                                    key={m.id}
                                    className="flex w-full cursor-pointer items-center gap-2 rounded-md border-none px-2.5 py-1.5 text-left text-xs font-semibold text-[var(--color-on-surface)] hover:bg-[var(--color-primary-bg)]"
                                >
                                    <span
                ...
                <div className="flex flex-col gap-0.75 py-1.5">
                    <span className="text-[11px] font-bold uppercase tracking-[0.04em] text-[var(--color-text-tertiary)]">Due Date</span>
                    <div className="relative flex cursor-pointer items-center gap-1.5 rounded-md px-1.5 py-1 text-[13px] font-medium text-[var(--color-text-secondary)] transition-colors hover:bg-[var(--color-primary-bg)]">
                        <Calendar size={14} className="shrink-0 text-[var(--color-text-tertiary)]" />
