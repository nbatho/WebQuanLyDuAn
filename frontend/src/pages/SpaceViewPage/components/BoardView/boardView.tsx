...
export default function BoardView({
    ...
    const dragItem = useRef<{ fromGroupId: string; taskId: number } | null>(null);
    ...
    const onDragStart = (e: DragEvent, groupId: string, taskId: number) => {
    ...
    return (
        <div className="flex flex-1 flex-col overflow-hidden">
            <div className="flex flex-1 items-start gap-3 overflow-x-auto px-5 py-4">
                        ...
                        >
                            <div className="flex items-center justify-between px-3.5 py-2.5">
                                <div className="flex items-center gap-1.75">
                                    ...
                                    />
                                    <span className="text-xs font-extrabold uppercase tracking-[0.04em] text-[#141b2b]">
                                        {group.name}
                                    </span>
                                    <span className="rounded-full bg-[#eef0f5] px-1.25 py-px text-[11px] font-bold text-[#5f6368]">
                                        {columnRoots.length}
                                    </span>
                                </div>
                                <div className="flex gap-0.5">
                                    <button className="flex items-center rounded p-0.75 px-1.25 text-[#9aa0a6] hover:bg-[#eef0f5] hover:text-[#5f6368]">
                                        <MoreHorizontal size={14} />
                                    </button>
                                    <button
                                        className="flex items-center rounded p-0.75 px-1.25 text-[#9aa0a6] hover:bg-[#eef0f5] hover:text-[#5f6368]"
                                        onClick={() => setInlineCol(group.id)}
                                    >
                                        <Plus size={14} />
                                    </button>
                                </div>
                            ...
                            <div className="flex max-h-[calc(100vh-260px)] flex-col gap-1.5 overflow-y-auto px-2 pb-2">
                                {columnRoots.map((task) => (
                                    <div
                                        key={task.task_id}
                                        className="cursor-pointer rounded-lg border border-[#eef0f5] bg-white px-3.5 py-3 shadow-[0_1px_3px_rgba(0,0,0,0.04)] transition-all duration-150 hover:-translate-y-px hover:border-[#dcdfe4] hover:shadow-[0_4px_12px_rgba(0,0,0,0.08)]"
                                        draggable
                                    ...
                                    >
                                        <div className="mb-1.5 text-[13px] font-semibold leading-[1.4] text-[#141b2b]">
                                            {task.name}
                                        </div>
                                        <div className="mb-2 text-[15px] text-[#dcdfe4]">
                                            <span>≡</span>
                                        </div>
                                ...
                                {inlineCol === group.id ? (
                                    ...
                                    <button
                                        className="flex w-full cursor-pointer items-center gap-1.5 rounded-md border-none bg-transparent px-1.5 py-1.75 text-left text-xs font-semibold text-[#9aa0a6] transition-all duration-150 hover:bg-[#f0f4ff] hover:text-[#0058be]"
                                        onClick={() => setInlineCol(group.id)}
                                    >
                ...
                <div className="h-fit cursor-pointer whitespace-nowrap rounded-lg px-3.5 py-2.5 text-[13px] font-semibold text-[#9aa0a6] transition-all duration-150 hover:bg-[#f0f4ff] hover:text-[#0058be]">
