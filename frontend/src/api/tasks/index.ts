...
export const createTask = async (space_id: number, taskData: Partial<TaskData>) : Promise<TaskData> => {
    return beApi.post(`/tasks/spaces/${space_id}`, taskData);
}

export const updateTask = async (task_id: number, updates: Partial<TaskData>) : Promise<TaskData> => {
...
// --- Attachments ---
export const getAttachmentsByTask = async (task_id: number) : Promise<any[]> => {
    return beApi.get(`/tasks/${task_id}/attachments`);
}

export const createAttachment = async (task_id: number, url: string, description?: string) : Promise<any> => {
